...
⚠️ 以上「优先 MCP」**仅适用于任务操作**（list_tasks / get_task / complete_task）。

⚠️ 任务来自 OpenClaw 写入的**文件队列**（`~/.openclaw/workspace/tasks/pending/`），不是内置 Cron/Task 系统。
